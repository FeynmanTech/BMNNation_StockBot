...
local mynick="FeynmanStockBot"
local n = mynick

local conf={}

local function loadpresets()
 local f=io.open"irclib.conf"
 if not f then
  f=io.open("irclib.conf","w")
  f:write('nick="'.."FeynmanStockBot"..[["
server="irc.freenode.net"
port=6667
autojoin={"#StockBotTesting"}
wcolor={255,255,0}
...
function irc_send(edit)
 --for _, edit in ipairs{...} do
	 edit = tostring(edit)
	 table.insert(history,1,edit)
	 cpos=0
	 if edit:match"^/" then
